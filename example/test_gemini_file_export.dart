...
void main() async {
  ...
  try {
    ...
    for (final category in GeminiFileCategory.values) {
      print('   📁 ${category.displayName}: ${category.fileTypes.length} types');
    }
    ...
    if (await catFile.exists()) {
      ...
      try {
        ...
        print('   🎨 Image generation: ${response.images.isNotEmpty ? "Success" : "No images"}');
        
        if (response.firstImage != null) {
          final outputFile = File('example/generated_images/export_test_output.png');
    ...
    for (final (fileName, fileType) in detectionTests) {
      final result = fileType != null ? '✅ ${fileType.description}' : '❌ Unknown';
      print('   $fileName → $result');
    }
    print('   ✅ File type detection working\n');

    print('🎉 === ALL EXPORTS WORKING PERFECTLY! ===');
    print('✅ GeminiFile is now available from package:gemini_dart/gemini_dart.dart');
